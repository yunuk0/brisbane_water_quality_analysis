...
if page == '프로젝트 개요':
    ...
    with st.expander("**프로젝트 개요**", expanded=True):
        st.markdown('''
        호주정부에서는 브리즈번 강의 수질을 주기적으로 모니터링하고, 그 데이터를 정부 오픈 데이터 포탈에 주기적으로 시민들에게 공개함으로써 누구나 강 주변을 안전하게 즐길 수 있도록 관리하고 있습니다.
        
        하지만 브리즈번 강은 조석의 영향을 강하게 받는 하천으로 물의 흐름과 수질 특성이 짧은 시간에도 크게 변해 시간 간격이 **일반적인 모니터링으로는 빠르게 변화하는 수질과 유속 패턴을 파악하기 어렵습니다**.
        
        본 프로젝트는 브리즈번 지역 주요 하천 및 하구에서 측정된 실측 수질 센서 데이터를 기반으로, 시간에 따라 변화하는 수질 상태를 진단하고 오염 위험을 예측하는 환경 데이터 사이언스 분석 프로젝트입니다.
    ...
    with st.expander("**브리즈번 지역 환경 배경**", expanded=True):
        ...
        st.write('''
        브리즈번 강은 길이 약 344km의 호주 퀸즐랜드주의 대표적인 하천으로, 도시 중심(Brisbane CBD)을 관통하며  도시 재생과 복원사업을 통해 산책로, 레저 활동, 관광 어트랙션 등이 활성화된 브리즈번의 핵심 생활·공간입니다.
        
        이처럼 강의 이용도가 높음과 동시에 조석·도시 유입·기후 영향이 복합적으로 작용해 수질 상태에 대한 정확한 정보는 시민의 안전과 도시 운영에 매우 중요합니다.
        ''')
