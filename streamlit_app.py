...
if page == '프로젝트 개요':
    ...
    with st.expander("**프로젝트 개요**", expanded=True):
        st.markdown('''
           호주정부에서는 브리즈번 강의 수질을 주기적으로 모니터링하고, 그 데이터를 주기적으로 시민들에게 공개함으로써 누구나 강 주변을 안전하게 즐길 수 있도록 관리하고 있습니다.
    ...
    with st.expander("**브리즈번 지역 환경 배경**", expanded=True):
        ...
        이처럼 강의 이용도가 높지만 조석·도시 유입·기후 영향이 복합적으로 작용해 수질 상태에 대한 정확한 정보는 시민의 안전과 도시 운영에 매우 중요합니다.
        ''')
